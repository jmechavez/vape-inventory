...
const MOVEMENT_FORM_TYPES: { type: FormType; label: string }[] = [
  ...
  { type: "adjustment", label: "Adjustment (Add/Remove/Loss)" },
];
  ...
  return (
    <div className="h-full flex flex-col gpu">
      ...
      {error && !showMovementModal && (
        <div className="rounded-xl border border-red-200 bg-red-50 px-5 py-3.5 text-base text-red-700 mb-3 shrink-0 animate-fade-in gpu">
          <div className="flex items-start justify-between gap-4">
            <span>{error}</span>
            <button type="button" className="shrink-0 font-medium underline" onClick={() => setError("")}>
              Dismiss
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        <div className="shrink-0 mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            <h2 className="text-base font-black text-zinc-950">Current Inventory</h2>
            {inventoryFilter !== "all" && (
              <button
                type="button"
                onClick={() => setInventoryFilter("all")}
                className="rounded-full bg-zinc-100 px-3 py-1 text-sm font-bold text-zinc-700 hover:bg-zinc-200 touch-feedback gpu"
              >
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <input
              ...
              className="w-full rounded-xl border border-zinc-300 bg-white px-4 py-3 text-base outline-none transition focus:border-black focus:ring-2 focus:ring-zinc-200 tap-target sm:w-72 gpu"
            />
            {search && (
              <button
                type="button"
                onClick={() => setSearch("")}
                className="rounded-xl border border-zinc-300 bg-white px-5 py-3 text-base font-bold text-zinc-700 transition hover:bg-zinc-100 tap-target touch-feedback gpu"
              >
                Clear
              </button>
            )}
        ...
        <div className="flex-1 overflow-y-auto rounded-2xl border border-zinc-200 bg-white shadow-sm gpu-scroll">
          {loadingInventory ? (
            ...
            <div className="p-8 text-center">
              ...
              {search && (
                <button onClick={() => setSearch("")} className="mt-3 rounded-lg border border-zinc-300 px-5 py-2.5 text-sm font-bold text-zinc-700 hover:bg-zinc-100 tap-target touch-feedback gpu">
                  Clear Search
                </button>
              )}
            </div>
            ...
            <div className="divide-y divide-zinc-100">
              {filteredInventory.map((item) => {
                ...
                return (
                  <div key={item.product_id} className="flex items-center justify-between p-5 hover:bg-zinc-50 card-hover gpu">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-3">
                        <h4 className="truncate text-lg font-black text-zinc-950">{item.name}</h4>
                        {item.version && (
                          <span className="rounded bg-zinc-200 px-2.5 py-1 text-xs font-bold uppercase text-zinc-700">
                            {item.version}
                          </span>
                        )}
                      </div>
      ...
      {showMovementModal && (
        ...
        >
          <div className="w-full max-w-2xl max-h-[90vh] overflow-hidden rounded-2xl bg-white shadow-2xl flex flex-col animate-slide-up gpu">
            ...
            <div className="border-b border-zinc-200 p-5 shrink-0">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h2 className="mt-0.5 text-xl font-black text-zinc-950">Record Stock Movement</h2>
                </div>
                <button
                  type="button"
            ...
            {/* Form Tabs */}
            <div className="flex flex-wrap gap-2 border-b border-zinc-200 px-5 py-3.5 shrink-0">
            ...
            <form onSubmit={submitMovement} className="flex-1 overflow-y-auto p-5 gpu-scroll">
              <div className="space-y-4">
                    ...
                    {suppliers.length === 0 && (
                      <p className="mt-1 text-base text-zinc-500">
                        ...
                        <button
                          type="button"
                          onClick={() => navigate("/suppliers")}
                          className="font-medium underline hover:text-zinc-900 touch-feedback gpu"
                        >
                          Add a supplier
                        </button>
                      </p>
                ...
                {activeForm === "receive" && (
                  <div>
                    ...
                    <input
                      ...
                      placeholder="0.00"
                      disabled={submitting}
                      className="w-full rounded-xl border border-zinc-300 bg-white px-4 py-3 font-mono text-base outline-none focus:border-black focus:ring-2 focus:ring-zinc-200 tap-target gpu"
                    />
                  </div>
                )}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    disabled={submitting}
                    className="w-full resize-none rounded-xl border border-zinc-300 bg-white px-4 py-3 text-base outline-none focus:border-black focus:ring-2 focus:ring-zinc-200 tap-target gpu"
                  />
                </div>
          ...
          <div className="w-full max-w-6xl max-h-[90vh] overflow-hidden rounded-2xl bg-white shadow-2xl flex flex-col animate-slide-up gpu">
            ...
            <div className="border-b border-zinc-200 p-5 shrink-0">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-bold uppercase tracking-[0.2em] text-zinc-500">Inventory</p>
                  <h2 className="mt-0.5 text-xl font-black text-zinc-950">Movement History</h2>
                </div>
                <div className="flex items-center gap-2.5">
                  ...
                  <select
                    ...
                    className="rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-base outline-none focus:border-black focus:ring-2 focus:ring-zinc-200 tap-target gpu"
                  >
                  ...
                  {filteredBySupplier.map((movement) => {
                    ...
                    return (
                      <div key={movement.id} className="flex items-center justify-between p-5 hover:bg-zinc-50 card-hover gpu">
                        ...
                        <div className="shrink-0 text-right">
                          <p className={`text-base font-bold ${incoming || positiveAdjustment ? "text-emerald-600" : "text-red-600"}`}>
                            {quantityText}
                          </p>
                          {hasUnitCost && (
            ...
            {!loadingMovements && filteredBySupplier.length > 0 && (
              <div className="border-t border-zinc-200 p-5 shrink-0 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
